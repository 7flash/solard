...
import { encryptKeypair, decryptKeypair, parseKeypair } from "../core/keypair.js";
import { measure } from "../core/log.js";
import { walletLog } from "../core/log-result.js";
import { measuredSync } from "../core/measured.js";
import { UnknownWalletError, WalletCannotSignError } from "../core/errors.js";
import type { WalletRef } from "../core/refs.js";
import type { SowlDatabase, WalletRow } from "./schema.js";

const m = measure("wallets");
function clean(ref: string): string { return ref.startsWith("@") ? ref.slice(1) : ref; }
...
export class WalletRepo {
  ...
  import(privateKey: string, name?: string): WalletRow {
    return measuredSync(m, "import", () => {
      ...
      const byAddress = this.db.wallets.select().where({ address }).first() as WalletRow | undefined;
      const byName = this.db.wallets.select().where({ name: resolvedName }).first() as WalletRow | undefined;
      ...
      return this.db.wallets.insert({
        name: resolvedName, address, ...encrypted, isActive: 1, createdAtMs: now, updatedAtMs: now,
      }) as WalletRow;
    }, walletLog);
  }

  list(): WalletRow[] {
    return this.db.wallets.select().where({ isActive: 1 }).orderBy("id", "asc").all() as WalletRow[];
  }

  resolve(ref: WalletRef): { address: PublicKey; row?: WalletRow; signer?: Keypair } {
    if (ref instanceof Keypair) return { address: ref.publicKey, signer: ref };
    if (ref instanceof PublicKey) {
      const row = this.db.wallets.select().where({ address: ref.toBase58() }).first() as WalletRow | undefined;
      return row ? { address: ref, row } : { address: ref };
    }
    if (typeof ref !== "string") return { address: new PublicKey(ref.address), row: ref };
    const value = clean(ref);
    const address = maybeAddress(value);
    const row = (address
      ? this.db.wallets.select().where({ address }).first()
      : this.db.wallets.select().where({ name: value }).first()) as WalletRow | undefined;
    if (!row) throw new UnknownWalletError(ref);
    return { address: new PublicKey(row.address), row };
  }

  signer(ref: WalletRef): { address: PublicKey; row?: WalletRow; signer: Keypair } {
    const resolved = this.resolve(ref);
    if (resolved.signer) return { ...resolved, signer: resolved.signer };
    if (!resolved.row) throw new WalletCannotSignError(resolved.address.toBase58());
