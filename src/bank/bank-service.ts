...
import { readMint } from "../chain/state.js";
...
import type { SendReceipt, SenderId, SimulationResult } from "../tx/types.js";
...
export class SowlBank {
  ...
  async sendToken(args: {
    ...
    if (!args.live) {
      ...
      const simulation = await this.sowl.simulatePlan(plan);
      return {
  ...
  async sweepDepositWallet(args: {
    ...
    const payer = this.sowl.signer(args.to);
  ...
  private resolveOwner(ref: WalletRef | string): PublicKey {
    ...
    try {
      return this.sowl.resolveWallet(ref).address;
    } catch {
