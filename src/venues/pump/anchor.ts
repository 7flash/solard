...
export function u64(value: bigint): Buffer {
  if (value < 0n || value > 0xffff_ffff_ffff_ffffn) throw new Error(`Invalid u64: ${value}`);
...
export function optionBool(value: boolean): Buffer { return Buffer.from([value ? 1 : 0]); }
