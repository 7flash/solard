...
import type { ClaimPlan, ClaimSourcePlugin, ClaimContext } from "../../claims/claim-source.js";
import { ata, ammCreatorVaultPda, creatorVaultPda, pumpSwapPoolPda, sharingConfigPda } from "./pda.js";
...
import { poolQuoteAsset, spendableVaultLamports, tokenAccountAmount } from "./common.js";
...
export class PumpCreatorFeesSource implements ClaimSourcePlugin {
  ...
  async resolveClaim(ctx: ClaimContext): Promise<ClaimPlan | null> {
    ...
    if (curve.complete) {
      const poolAddress = ctx.token.pool ? new PublicKey(ctx.token.pool) : pumpSwapPoolPda(mint, curve.quoteAsset.mint);
      const pool = await fetchPool(ctx.connection, poolAddress);
      quoteAsset = await poolQuoteAsset(ctx.connection, ctx.token, pool.quoteMint);
    ...
    if (quoteAsset.kind === "native-sol") {
      pumpVaultRaw = await spendableVaultLamports(ctx.connection, creatorVaultPda(vaultOwner));
    } else {
      const vaultAta = ata(quoteAsset.mint, creatorVaultPda(vaultOwner), quoteAsset.tokenProgram, true);
      try { pumpVaultRaw = await tokenAccountAmount(ctx.connection, vaultAta, quoteAsset.tokenProgram); } catch { /* no funded vault yet */ }
    }
    if (includeAmm && coinCreator) {
      const ammVaultAta = ata(quoteAsset.mint, ammCreatorVaultPda(coinCreator), quoteAsset.tokenProgram, true);
      try { ammVaultRaw = await tokenAccountAmount(ctx.connection, ammVaultAta, quoteAsset.tokenProgram); } catch { /* no funded vault yet */ }
    }
    ...
    const estimatedClaimRaw = pumpVaultRaw + (shared ? ammVaultRaw : quoteAsset.kind === "native-sol" ? 0n : ammVaultRaw);
    const userShare = sharing?.shareholders.find((holder) => holder.address.equals(ctx.user));
    const directToUser = !shared && curve.creator.equals(ctx.user) && (!coinCreator || coinCreator.equals(ctx.user));
    const spendableByUserRaw = shared
      ? userShare ? (estimatedClaimRaw * BigInt(userShare.shareBps)) / 10_000n : 0n
      : directToUser ? pumpVaultRaw : 0n;
    const nonSpendableClaimRaw = shared
      ? estimatedClaimRaw - spendableByUserRaw
      : quoteAsset.kind === "native-sol" ? ammVaultRaw : 0n;
    ...
    return {
      ...
      instructions: buildClaimInstructions({
        ...
        coinCreator,
        shareholderAddresses: sharing?.shareholders.map((holder) => holder.address),
      }),
      estimatedClaimRaw,
      spendableByUserRaw,
      meta: {
        path: shared ? "sharing-config" : includeAmm ? "direct-curve-and-amm" : "direct-curve",
        sharingConfig: shared,
        includeAmm,
        ...
        shareholders: sharing?.shareholders.map((holder) => ({ address: holder.address.toBase58(), shareBps: holder.shareBps })) ?? null,
      },
    };
  }
}
