...
import { configuredTotalFeeBps, defaultPumpQuoteShell, type CurveMarketMeta } from "./common.js";
...
export class PumpCurveVenue implements TradeVenuePlugin {
  ...
  async inspectToken(connection: Connection, mint: PublicKey): Promise<Partial<TokenRow> | null> {
    const now = Date.now();
    const curve = await fetchCurve(connection, defaultPumpQuoteShell(mint, now));
    ...
    return {
      ...
      sharingConfig: sharing ? sharingConfigPda(mint).toBase58() : null, refreshedAtMs: now,
    };
  }
  ...
  async resolveMarket(ctx: VenueContext): Promise<VenueMarket | null> {
    ...
    return {
      venue: this.id, mint: new PublicKey(ctx.token.mint), quoteAsset: curve.quoteAsset,
      baseTokenProgram: defaultTokenProgram(ctx.token), creator: curve.creator, metadata: { curve } satisfies CurveMarketMeta,
    };
  }

  async quoteBuy(ctx: VenueContext, market: VenueMarket, amount: RawAmount, slippageBps: number): Promise<QuoteResult> {
    if (!sameAsset(market.quoteAsset, amount.asset)) throw new Error("Buy amount asset does not match Pump curve quote asset");
    const curve = (market.metadata as CurveMarketMeta).curve;
    const value = quoteBuyConstantProduct(amount, curve, slippageBps, configuredTotalFeeBps(ctx.token));
  ...
  async quoteSell(ctx: VenueContext, market: VenueMarket, amountRaw: bigint, slippageBps: number): Promise<QuoteResult> {
    const curve = (market.metadata as CurveMarketMeta).curve;
    const value = quoteSellConstantProduct(amountRaw, curve, slippageBps, configuredTotalFeeBps(ctx.token));
  ...
  async price(ctx: VenueContext, market: VenueMarket): Promise<MarketPrice> {
    ...
    return {
      ...
      priceQuotePerToken: spotPriceQuotePerToken(curve, ctx.token.decimals ?? 6, market.quoteAsset.decimals),
      baseReserveRaw: curve.virtualBase, quoteReserveRaw: curve.virtualQuote, capturedAtMs: Date.now(),
    };
  }
  ...
  async buildBuy(ctx: VenueContext, market: VenueMarket, quote: QuoteResult): Promise<BuiltInstructions> {
    ...
    const curve = (market.metadata as CurveMarketMeta).curve;
    const routing = await resolvePumpRouting(ctx.connection, ctx.token, market.creator, market.quoteAsset, curve.isMayhemMode);
