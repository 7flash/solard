...
async function readConfig(cwd: string): Promise<SowlConfig> {
  for (const name of CONFIG_FILES) {
    ...
    )) as { default?: SowlConfig };
    return module.default ?? {};
  }
  return {};
}

function pathFromEntry(entry: SowlScriptEntry): {
  path: string;
  description?: string;
} {
...
export async function resolveScript(
  ...
  throw new Error(
    `Unknown script: ${ref}. Use a file path, create scripts/${ref}.ts, or register it in sowl.config.ts.`,
  );
}
