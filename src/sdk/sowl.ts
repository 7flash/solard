...
export class Sowl implements ComposerHost {
  ...
  registerSender(sender: SowlSender): this {
  ...
  async preparePendingBuy(
    ...
    deployment: PreparedTokenDeployment,
    buyer: WalletRef,
    amount: import("../core/amounts.js").RawAmount,
  ...
  agent(name: string, wallet?: WalletRef): SowlAgent {
    ...
    if (!resolvedWallet)
      throw new Error(
        `Agent ${name} has no wallet. Configure it with sowl agent create ${name} --wallet <wallet>`,
      );
