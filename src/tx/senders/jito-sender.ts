...
export class JitoSender implements SowlBundleSender {
  ...
  async send({
    connection,
    transaction,
  }: Parameters<SowlBundleSender["send"]>[0]): Promise<string> {
  ...
  async sendBundle({
    transactions,
  }: Parameters<SowlBundleSender["sendBundle"]>[0]) {
    ...
    if (!response.ok || data.error || !data.result)
      throw new Error(
        `Jito bundle submission failed: ${JSON.stringify(data.error ?? response.status)}`,
      );
    return {
      submissionId: data.result,
      signatures: transactions.map(transactionSignature),
    };
  }
}
