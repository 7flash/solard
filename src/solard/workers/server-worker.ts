...
let serverHandle: { stop?: (closeActiveConnections?: boolean) => unknown } | null = null;
...
try {
  serverHandle = await serve({
    port: PORT,
    routes: {
      "/health": () => new Response(JSON.stringify({ status: "ok", time: Date.now() }), { headers: { "Content-Type": "application/json" } }),
      "/metrics": () => new Response(JSON.stringify({ pid: process.pid, memory: process.memoryUsage() }), { headers: { "Content-Type": "application/json" } })
    }
  }) as any;

  console.log(`[${WORKER_NAME}] Tradjs web server listening natively on port ${PORT}`);
  sendHeartbeat("running");

  // 2. High-Resolution Heartbeat Ping Interval Loop
  heartbeatTimer = setInterval(() => sendHeartbeat("running"), 3000);
} catch (error: any) {
  sendHeartbeat("error", error instanceof Error ? error.message : String(error));
...
const shutdown = async (signal: string) => {
  console.log(`[${WORKER_NAME}] Intercepted ${signal}, tearing down server contexts...`);
  if (heartbeatTimer) clearInterval(heartbeatTimer);
  
  sendHeartbeat("stopped");
  
  try { if (serverHandle?.stop) serverHandle.stop(true); } catch (e) { console.error(e); }
