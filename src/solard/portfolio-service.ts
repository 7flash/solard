...
function groupNamesByWallet(sowl: Sowl): Map<string, string[]> {
  const out = new Map<string, string[]>();
  for (const group of sowl.groups.list()) {
    const name = String(group.name ?? "").trim();
    if (!name) continue;
    for (const member of sowl.groups.wallets(name)) {
