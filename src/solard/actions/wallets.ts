...
export function importWalletAction(
  ...
  return compactWallet(
    ctx.sowl.importWallet(privateKey, input.name?.trim() || undefined),
  );
}
...
export async function listWalletsAction(
  ...
  >
> {
  const wallets = ctx.sowl.wallets.list();
  const selectedTokens = input.token
    ? [ctx.sowl.resolveToken(input.token)]
    : ctx.sowl.tokens.list();
  const rows = [] as Array<
    WalletSummary & {
      solLamports?: string;
      tokenBalances?: Array<Record<string, unknown>>;
  ...
  for (const wallet of wallets) {
    ...
    try {
      const balances = await ctx.sowl.walletBalances(wallet, selectedTokens);
      rows.push({
        ...base,
