...
import type { SenderId } from "../../tx/types.js";
...
export function normalizeTradeTargets(
  ...
  return {
    mode: "group",
    refs: ctx.sowl.groupWallets(group!).map((ref) => String(ref)),
    group,
  };
}
...
export async function buyTokenAction(
  ...
  if (!input.live) {
    ...
    const results = await Promise.all(
      plans.map((plan) => ctx.sowl.simulatePlan(plan)),
    );
  ...
  if (via === "helius-fast" && targets.refs.length === 1) {
    const payer = ctx.sowl.signer(targets.refs[0]!).publicKey;
  ...
  const receipt =
    targets.refs.length === 1
      ? await ctx.sowl.buy(token, targets.refs[0]!, sol(amountSol), options)
      : await ctx.sowl.buyMany(token, targets.refs, sol(amountSol), options);
...
export async function sellTokenAction(
  ...
  if (!input.live) {
    const plans =
      targets.refs.length === 1
        ? [await ctx.sowl.tx(targets.refs[0]!).sell(token, options).build()]
        : await ctx.sowl.composeMany(targets.refs).sell(token, options).build();
    const results = await Promise.all(
      plans.map((plan) => ctx.sowl.simulatePlan(plan)),
    );
