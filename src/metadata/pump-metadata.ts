...
function contentType(imagePath: string): string {
  switch (extname(imagePath).toLowerCase()) {
    ...
    default:
      throw new Error(
        `Unsupported image extension for Pump metadata: ${extname(imagePath) || "(none)"}`,
      );
  }
}
function ipfsUri(cid: string): string {
  const gateway = (
    process.env.IPFS_PUBLIC_GATEWAY ?? DEFAULT_PUBLIC_GATEWAY
  ).replace(/\/$/, "");
  return `${gateway}/${cid}`;
}
...
function validateInput(input: PumpCoinMetadataInput): {
  imagePath: string;
  bytes: Uint8Array;
  mime: string;
} {
  ...
  if (!existsSync(imagePath))
    throw new Error(`Metadata image file not found: ${imagePath}`);
  ...
  if (!input.description.trim())
    throw new Error(
      `Metadata description cannot be empty for $${input.symbol}`,
    );
  const stat = statSync(imagePath);
  const maxBytes = Number(
    process.env.SOWL_MAX_IMAGE_BYTES ??
      process.env.PINATA_MAX_IMAGE_BYTES ??
      DEFAULT_MAX_IMAGE_BYTES,
  );
  if (stat.size > maxBytes)
    throw new Error(
      `Metadata image is too large: ${stat.size} bytes exceeds ${maxBytes}`,
    );
  return {
    imagePath,
    bytes: new Uint8Array(readFileSync(imagePath)),
    mime: contentType(imagePath),
  };
}
...
function baseMetadata(input: PumpCoinMetadataInput): PumpCoinMetadataJson {
  return {
    ...
    ...(optional(input.createdOn)
      ? { createdOn: optional(input.createdOn)! }
      : {}),
    ...(optional(input.twitter) ? { twitter: optional(input.twitter)! } : {}),
    ...(optional(input.telegram)
      ? { telegram: optional(input.telegram)! }
      : {}),
...
function resultLog(uploaded: UploadedPumpCoinMetadata) {
  return {
    ...
    symbol: uploaded.metadata.symbol,
...
export async function uploadPumpMetadataWithPumpFrontend(
  ...
  return await measured(
    m,
    `pump-frontend $${input.symbol}`,
    async () => {
      const { imagePath, bytes, mime } = validateInput(input);
      const endpoint =
        options.endpoint ??
        process.env.PUMP_IPFS_ENDPOINT?.trim() ??
        DEFAULT_PUMP_FRONTEND_ENDPOINT;
      const form = new FormData();
      form.append(
        "file",
        new Blob([blobPart(bytes)], { type: mime }),
        basename(imagePath),
      );
      ...
      if (!response.ok)
        throw new Error(
          `Pump frontend metadata upload failed (${response.status}): ${body.slice(0, 300)}`,
        );
      let data: {
        metadataUri?: unknown;
        metadata?: PumpCoinMetadataJson;
      ...
      try {
        ...
        throw new Error(
          `Pump frontend metadata upload returned non-JSON: ${body.slice(0, 300)}`,
        );
      }
      if (typeof data.metadataUri !== "string" || !data.metadataUri.trim()) {
        throw new Error(
          `Pump frontend metadata upload returned no metadataUri: ${body.slice(0, 300)}`,
        );
      }
      return {
...
export async function uploadPumpMetadataWithPinata(
  ...
  return await measured(
    ...
    async () => {
      const { imagePath, bytes, mime } = validateInput(input);
      const jwt = requiredEnv(
        "PINATA_JWT",
        "Create a Pinata JWT and set it before using --provider pinata.",
      );
      ...
      const imageFile = new File([blobPart(bytes)], basename(imagePath), {
        type: mime,
      });
      const imageUpload = await pinata.upload.public
        .file(imageFile)
        .name(`${input.symbol}-image-${basename(imagePath)}`);
      const imageUri = ipfsUri(imageUpload.cid);
      const metadata: PumpCoinMetadataJson = {
        ...baseMetadata(input),
        image: imageUri,
        createdOn: input.createdOn ?? "https://pump.fun",
      };
      const metadataUpload = await pinata.upload.public
        .json(metadata)
        .name(`${input.symbol}-metadata.json`);
      return {
...
export async function uploadPumpMetadata(
  ...
  try {
    return await uploadPumpMetadataWithPumpFrontend(input, {
      endpoint: options.endpoint,
    });
  } catch (error) {
    const fallback =
      options.fallback ??
      (process.env.SOWL_METADATA_FALLBACK === "pinata" ? "pinata" : null);
