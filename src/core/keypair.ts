...
export function parseKeypair(raw: string): Keypair {
  ...
  if (source.startsWith("[")) {
    ...
    if (!Array.isArray(bytes) || bytes.some((n) => !Number.isInteger(n) || n < 0 || n > 255)) {
...
function masterKey(): Buffer {
  ...
  if (!source) throw new MissingConfigError("SOWL_MASTER_KEY (required for importing or signing with persisted wallets)");
  return createHash("sha256").update(source).digest();
}
export type EncryptedSecret = { encryptedSecretKey: string; nonce: string; authTag: string };
export function encryptKeypair(keypair: Keypair): EncryptedSecret {
  ...
  return { encryptedSecretKey: encrypted.toString("base64"), nonce: nonce.toString("base64"), authTag: cipher.getAuthTag().toString("base64") };
}
export function decryptKeypair(secret: EncryptedSecret): Keypair {
  const decipher = createDecipheriv("aes-256-gcm", masterKey(), Buffer.from(secret.nonce, "base64"));
  decipher.setAuthTag(Buffer.from(secret.authTag, "base64"));
  return Keypair.fromSecretKey(Buffer.concat([decipher.update(Buffer.from(secret.encryptedSecretKey, "base64")), decipher.final()]));
}
