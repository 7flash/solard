...
import type { PlannedTransaction, SimulationResult } from "../tx/types.js";
...
export async function simulatePlanned(connection: Connection, plan: PlannedTransaction): Promise<SimulationResult> {
  let result: SimulationResult | undefined;
  await m.measure("transaction", async () => {
    const addresses = plan.draft.trackedAccounts.map((a) => a.address.toBase58());
    const before = addresses.length ? await connection.getMultipleAccountsInfo(plan.draft.trackedAccounts.map((a) => a.address), "confirmed") : [];
    ...
    const accountChanges = addresses.map((address, index) => {
      ...
      return { address, beforeLamports: pre, afterLamports: post, deltaLamports: pre == null || post == null ? null : post - pre };
    });
    const tokenChanges = plan.draft.trackedAccounts.filter((a) => a.kind === "token").map((tracked) => {
      ...
      const afterData = Array.isArray(rawAfterData) && rawAfterData[0]
        ? Buffer.from(rawAfterData[0], "base64")
        : undefined;
      const pre = tokenRaw(preData), post = tokenRaw(afterData);
      return {
        address: tracked.address.toBase58(), mint: tracked.mint?.toBase58() ?? null,
        beforeRaw: pre?.toString() ?? null, afterRaw: post?.toString() ?? null,
        deltaRaw: pre != null && post != null ? (post - pre).toString() : null,
      };
    });
    result = {
      ...
      accountChanges,
      tokenChanges,
      solChanges: accountChanges.filter((row) => plan.draft.trackedAccounts.find((a) => a.address.toBase58() === row.address)?.kind === "sol"),
    };
    return {
