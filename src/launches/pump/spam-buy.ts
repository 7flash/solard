...
function placeholderToken(sowl: Sowl, mint: PublicKey): TokenRow {
  return sowl.tokens.upsert({
    mint: mint.toBase58(),
    decimals: 6,
...
async function waitForPumpMarket(args: {
  ...
  let token = placeholderToken(args.sowl, args.mint);
  ...
  while (!args.signal?.aborted) {
    ...
    try {
      args.sowl.cache.invalidate();
      token = await args.sowl.refreshToken(token);
...
async function buildLivePlan(args: {
  ...
  args.sowl.cache.invalidate();
  const token = await args.sowl.refreshToken(args.token);
  const builder = args.sowl.tx(args.buyer.walletRef);
...
async function signatureStates(
  ...
  const statuses = (
    await sowl.connection().getSignatureStatuses([...signatures], {
...
async function runBuyerLoop(args: {
  ...
  while (!args.signal?.aborted) {
    ...
    if (signatures.length > 0) {
      try {
        const states = await signatureStates(args.sowl, signatures);
    ...
    try {
      if (needsFreshQuote) {
        ...
        active = await args.sowl.compile(
          args.sowl.signer(args.buyer.walletRef),
          active.draft,
          { useAlts: false },
        );
      ...
      const submitted: SubmittedPlan = await args.sowl.broadcastPlan(
        active,
        settings.sender,
