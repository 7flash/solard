...
import { MissingConfigError } from "./errors.ts";
...
function masterKey(): Buffer {
  const source = process.env.SLRD_MASTER_KEY;
  if (!source)
    throw new MissingConfigError(
      "SLRD_MASTER_KEY (required for importing or signing with persisted wallets)",
    );
