...
import type { SolardConfig, SolardScriptEntry } from "./config.ts";
...
async function readConfig(cwd: string): Promise<SolardConfig> {
  for (const name of CONFIG_FILES) {
    const path = resolve(cwd, name);
    if (!existsSync(path)) continue;
    const module = (await import(
      `${pathToFileURL(path).href}?t=${Date.now()}`
    )) as { default?: SolardConfig };
    return module.default ?? {};
  }
  return {};
}

function pathFromEntry(entry: SolardScriptEntry): {
  path: string;
  description?: string;
} {
...
export async function resolveScript(
  ...
  if (configured) {
    ...
    if (!existsSync(path))
      throw new Error(`Configured script '${ref}' does not exist: ${path}`);
  ...
  throw new Error(
    `Unknown script: ${ref}. Use a file path, create scripts/${ref}.ts, or register it in slrd.config.ts.`,
  );
}
