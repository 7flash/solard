...
import { LaunchTimeoutError } from "../../core/errors.ts";
import type {
  DiscoveredLaunch,
  LaunchFilter,
  LaunchSourcePlugin,
...
class Cursor {
  ...
  string(): string {
    if (this.offset + 4 > this.data.length)
      throw new Error("Invalid Pump create data: missing string length");
    const length = this.data.readUInt32LE(this.offset);
    this.offset += 4;
    if (this.offset + length > this.data.length)
      throw new Error("Invalid Pump create data: truncated string");
  ...
  pubkey(): PublicKey {
    if (this.offset + 32 > this.data.length)
      throw new Error("Invalid Pump create data: missing creator");
...
function exactFilter(
  ...
  if (
    ...
    candidate.symbol?.toUpperCase() !==
      filter.symbol.replace(/^\$/, "").toUpperCase()
  )
    return false;
  if (expectedCreator && candidate.creator?.toBase58() !== expectedCreator)
    return false;
  if (expectedQuote && candidate.token.quoteMint !== expectedQuote)
    return false;
  return true;
}
...
function decodeCreateInstruction(args: {
  ...
  const quoteMint =
    isV2 && args.instruction.accounts[16]
      ? args.instruction.accounts[16]!
      : WRAPPED_SOL_MINT;
  const quoteProgram =
    isV2 && args.instruction.accounts[18]
  ...
  return {
    ...
    token: {
      ...
      metadataJson: JSON.stringify({
        launchSignature: args.signature,
        launchSource: "pump-launch",
      }),
    },
...
export class PumpLaunchSource implements LaunchSourcePlugin {
  ...
  async waitForLaunch(
    ...
    const commitment: Finality =
      watchCommitment === "finalized" ? "finalized" : "confirmed";
    ...
    return await new Promise<DiscoveredLaunch>((resolve, reject) => {
      ...
      if (args.signal?.aborted) {
        fail(new Error("Launch wait aborted"));
        return;
      }
      args.signal?.addEventListener("abort", aborted, { once: true });
      timeout = setTimeout(
        () => fail(new LaunchTimeoutError(this.id, timeoutMs)),
        timeoutMs,
      );
      try {
        subscriptionId = connection.onLogs(
          ...
          async (logInfo) => {
            ...
            try {
              const launch = await this.candidate(
                connection,
                logInfo.signature,
                commitment,
              );
