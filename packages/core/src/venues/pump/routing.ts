...
import { ata, ammGlobalConfigPda, creatorVaultPda, globalPda } from "./pda.ts";
...
export function tokenMeta(token: TokenRow): Record<string, unknown> {
  ...
  try {
    return JSON.parse(token.metadataJson) as Record<string, unknown>;
  } catch {
    return {};
  }
}
...
function choose(list: PublicKey[], mint: PublicKey): PublicKey {
  return list[mint.toBytes()[0]! % list.length]!;
}
export async function resolvePumpRouting(
  ...
  const nested =
    typeof meta.pumpRouting === "object" && meta.pumpRouting
      ? (meta.pumpRouting as Record<string, unknown>)
      : {};
  ...
  if (!feeRecipient || !buybackFeeRecipient) {
    ...
    if (!account || account.data.length < 997)
      throw new Error("Unable to decode live Pump global fee routing");
    const data = Buffer.from(account.data);
    const regularFees = [
      key(data, 41),
      ...Array.from({ length: 7 }, (_, i) => key(data, 162 + i * 32)),
    ];
    const reservedFees = [
      key(data, 483),
      ...Array.from({ length: 7 }, (_, i) => key(data, 516 + i * 32)),
    ];
    const buybacks = Array.from({ length: 8 }, (_, i) =>
      key(data, 741 + i * 32),
    );
    feeRecipient ??= choose(
      isMayhemMode ? reservedFees : regularFees,
  ...
  return {
    feeRecipient,
    buybackFeeRecipient,
    associatedQuoteFeeRecipient: ata(
      quote.mint,
      feeRecipient,
      quote.tokenProgram,
      true,
    ),
    associatedQuoteBuybackFeeRecipient: ata(
      quote.mint,
      buybackFeeRecipient,
      quote.tokenProgram,
      true,
    ...
    creatorVault: vault,
    associatedCreatorVault: ata(quote.mint, vault, quote.tokenProgram, true),
  };
}
export async function resolvePumpSwapProtocolFeeRecipient(
  connection: Connection,
  mint: PublicKey,
  isMayhemMode: boolean,
  override?: string,
  ...
  if (!account || account.data.length < 642)
    throw new Error("Unable to decode live PumpSwap global fee routing");
  const data = Buffer.from(account.data);
  const regular = Array.from({ length: 8 }, (_, i) => key(data, 57 + i * 32));
  const reserved = [
    key(data, 385),
    ...Array.from({ length: 7 }, (_, i) => key(data, 418 + i * 32)),
  ];
