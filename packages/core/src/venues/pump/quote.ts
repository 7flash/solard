import type { RawAmount } from "../../core/amounts.ts";
export type CurveReserves = { virtualBase: bigint; virtualQuote: bigint };
function bpsFloor(value: bigint, bps: number): bigint {
  return (value * BigInt(bps)) / 10_000n;
}
export function quoteBuyConstantProduct(
  amount: RawAmount,
  ...
  return {
    ...
    minimumOutputRaw: min,
  };
}
export function quoteSellConstantProduct(
  inputRaw: bigint,
  reserves: CurveReserves,
  slippageBps: number,
  totalFeeBps = 200,
  ...
  const gross =
    (inputRaw * reserves.virtualQuote) / (reserves.virtualBase + inputRaw);
...
export function spotPriceQuotePerToken(
  reserves: CurveReserves,
  baseDecimals: number,
  quoteDecimals: number,
): number {
  if (reserves.virtualBase <= 0n || reserves.virtualQuote <= 0n)
    throw new Error("Cannot calculate price from empty reserves");
