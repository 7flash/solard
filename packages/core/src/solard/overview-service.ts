...
export async function loadSolardOverview(
  ...
  const rawGroups = slrd.groups.list().map((group: any) => ({
    ...group,
    wallets: slrd.groups.wallets(group.name),
  }));
  const walletGroups = groupNamesByWallet(rawGroups);
  const wallets = slrd.wallets
    .list()
    .map((wallet: any) => {
  ...
  const tokens = slrd.tokens.list().slice(0, tokenLimit);
  const executions = slrd.executions.history(executionLimit);
