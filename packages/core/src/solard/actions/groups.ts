...
export function createGroupAction(
  ...
  return ctx.slrd.groups.create(name, input.description?.trim() || undefined);
}
...
export function addWalletToGroupAction(
  ...
  ctx.slrd.groups.create(group);
  const resolved = ctx.slrd.resolveWallet(wallet);
  return ctx.slrd.groups.addWallet(
    group,
...
export function showGroupAction(
  ...
  const group = input.group?.trim();
  if (!group) throw new Error("group is required");
  const members = ctx.slrd.groups.wallets(group).map((row) => {
    const wallet = ctx.slrd.resolveWallet(row.walletAddress);
    return {
