...
import { parsePumpLogs } from "../helius/pump-log-events.ts";
import {
...
function redact(value: string): string {
  return value
    .replace(/(api-key|apiKey)=([^&]+)/gi, "$1=<redacted>")
...
export function heliusRpcUrl(): string {
  ...
  throw new Error(
    "Missing Helius RPC config. Set HELIUS_RPC_URL, RPC_ENDPOINT, SOLANA_RPC_URL, or HELIUS_API_KEY.",
  );
}
...
async function rpc(
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(
        `RPC ${method} HTTP ${res.status}: ${JSON.stringify(body)?.slice(0, 300)}`,
      );
    if (body?.error)
      throw new Error(
...
async function checkSqlite(
  ...
  return {
    dbPath: process.env.SLRD_DB_PATH ?? process.env.SOLARD_DB_PATH ?? null,
    writeProbe,
    probe,
    feedRows: rows.length,
    newestMint: rows[0]?.mint ?? null,
  };
}

async function checkHeliusHttp(
  sampleTransaction: boolean,
): Promise<Record<string, unknown>> {
  const solUsd = await resolveSolUsd().catch(() => null);
  const signatures = await rpc("getSignaturesForAddress", [
    PUMPFUN_PROGRAM_ID,
    { limit: sampleTransaction ? 8 : 1 },
  ]);
  const first = Array.isArray(signatures)
    ? signatures.find((row) => row?.signature)?.signature
    : null;
...
async function checkHeliusLogs(
  seconds: number,
): Promise<Record<string, unknown>> {
  const WebSocketCtor = globalThis.WebSocket;
  if (!WebSocketCtor)
    throw new Error("globalThis.WebSocket is unavailable in this runtime");
  ...
  await new Promise<void>((resolve, reject) => {
      ...
      ws.send(
        JSON.stringify({
          ...
          params: [
            ...
            {
              commitment:
                process.env.SOLARD_HELIUS_LOGS_COMMITMENT ?? "processed",
            },
          ],
        }),
      );
    ...
    ws.addEventListener("message", (event: any) => {
      ...
      try {
        ...
        counters.programDataLogs += logs.filter((line: string) =>
    ...
    ws.addEventListener("error", (event: any) => {
      clearTimeout(timer);
      reject(
        new Error(String(event?.message ?? "Helius logs websocket error")),
      );
    });
  });
...
async function checkPumpPortal(
  seconds: number,
): Promise<Record<string, unknown>> {
  const WebSocketCtor = globalThis.WebSocket;
  if (!WebSocketCtor)
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    ws.addEventListener("message", (event: any) => {
      ...
      try {
        ...
        counters.lastRaw = {
          keys: Object.keys(msg).slice(0, 16),
          txType: msg?.txType ?? msg?.type ?? null,
          hasMint: !!mint,
          bytes: raw.length,
        };
...
function recommendations(checks: CheckResult[]): string[] {
  ...
  if (!sqlite?.ok)
    out.push(
      "SQLite is failing before stream data can render. Fix DB path/schema first.",
    );
  if (heliusLogs && !heliusLogs.ok)
    out.push(
      "Helius logs WebSocket is not connecting. Check HELIUS_API_KEY / HELIUS_WS_URL and plan permissions.",
    );
  if (heliusLogs?.ok && Number(heliusLogs.data?.received ?? 0) === 0)
    out.push(
      "Helius WebSocket opened but received no messages during the sample window. Try a longer window or confirmed/processed commitment toggle.",
    );
  if (
    heliusLogs?.ok &&
    Number(heliusLogs.data?.notifications ?? 0) > 0 &&
    Number(heliusLogs.data?.decodedCreates ?? 0) +
      Number(heliusLogs.data?.decodedTrades ?? 0) ===
      0
  )
    out.push(
      "Helius delivered Pump logs but parser decoded no Pump events. Capture lastRaw/log sample and update event layout/discriminators.",
    );
  if (heliusHttp && !heliusHttp.ok)
    out.push(
      "Helius HTTP RPC failed. The polling fallback cannot work until RPC config is fixed.",
    );
  if (pump && pump.ok && Number(pump.data?.received ?? 0) === 0)
    out.push(
      "PumpPortal WebSocket opened but delivered no messages during the sample window.",
    );
  if (out.length === 0)
    out.push(
      "Transport and DB checks passed. If UI is blank, the issue is likely feed filters, source selection, or row mapping.",
    );
  return out;
}
...
export async function runLiveDoctor(
  ...
  if (source === "helius" || source === "both") {
    checks.push(
      await timed("helius.httpRpc", () => checkHeliusHttp(sampleTransaction)),
    );
    checks.push(
      await timed("helius.logsSubscribe", () => checkHeliusLogs(seconds)),
    );
  }
