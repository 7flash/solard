...
import { UnknownTokenError } from "../core/errors.ts";
import { tokenLog } from "../core/log-result.ts";
import { measuredSync } from "../core/measured.ts";
import type { TokenRef } from "../core/refs.ts";
...
export class TokenRepo {
  ...
  upsert(input: Partial<Omit<TokenRow, "id">> & { mint: string }): TokenRow {
    return measuredSync(
      m,
      `upsert ${input.mint.slice(0, 8)}`,
      () => {
        const existing = this.db.tokens
          .select()
  ...
  resolve(ref: TokenRef): TokenRow {
    ...
    const value =
      ref instanceof PublicKey ? ref.toBase58() : normalizeName(ref);
    ...
    const token = isMint(value)
      ...
      : rows.find(
          (row) =>
            row.name === value ||
            row.symbol?.toUpperCase() === value.toUpperCase(),
        );
