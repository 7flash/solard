...
import { rawAmount, SOL_ASSET, sol } from "../../core/amounts.ts";
...
function walletRefsFromFlags(slrd: Solard, flags: Flags): WalletRef[] {
  ...
  const refs = group ? slrd.groupWallets(group) : (wallets as WalletRef[]);
  if (refs.length === 0) {
  ...
  return refs.filter((ref) => {
    const address = slrd.resolveWallet(ref).address.toBase58();
...
async function prepareBuyer(
  ...
  const balanceLamports = BigInt(
    await slrd.connection().getBalance(wallet.address, "confirmed"),
  );
...
export async function runPumpBuyerSpamFromArgs(
  ...
  try {
    const refs = walletRefsFromFlags(slrd, flags);
    const buyers = await Promise.all(
      refs.map((ref) => prepareBuyer(slrd, ref, amount)),
    );
