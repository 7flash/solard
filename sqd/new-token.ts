...
export type CreateInstructionKind =
  | "create"
  | "create_v2";
...
const REQUEST_TIMEOUT_MS = positiveIntEnv(
  "SQD_REQUEST_TIMEOUT_MS",
  60_000,
);

const DEDUPE_SIZE = positiveIntEnv(
  "SQD_DEDUPE_SIZE",
  100_000,
);

const REORG_REWIND = positiveIntEnv(
  "SQD_REORG_REWIND",
  128,
);

const LOG_QUERY =
  process.env.SQD_LOG_QUERY === "1";
...
export function base58Decode(
  ...
  for (
    let index = leadingOnes;
    index < value.length;
    index++
  ) {
    const digit = B58_INDEX.get(
      value[index]!,
    );
...
class BorshReader {
  ...
  string(): string {
    ...
    if (length > 1_000_000) {
      throw new Error(
        `unreasonable string length ${length}`,
      );
    }
  ...
  const timer = setInterval(() => {
    m.sync(
      ...
      () => ({
        ...
        lastRequestAgeMs:
          ageMs(
            status.lastRequestAtMs,
          ),
        lastResponseAgeMs:
          ageMs(
            status.lastResponseAtMs,
          ),
        lastBlockAgeMs:
          ageMs(
...
class PortalForkError extends Error {
  constructor(
    readonly previousBlocks:
      HeaderPoint[],
  ) {
    super(
      "SQD Portal reported a hot-chain fork",
    );
  ...
  return await m(
    ...
    async () => {
      ...
      if (
        ...
        throw new Error(
          `Portal ${endpoint} returned no valid head`,
        );
      }
...
async function requestStreamBatch(
  ...
  return await m(
    ...
    async () => {
      ...
      if (!response.body) {
        throw new Error(
          "Portal returned 200 without a response body",
        );
      }
      ...
      let lastSlot:
        number | undefined;
      let lastHeader:
        HeaderPoint | undefined;

      for await (
        const item of readNdjson(
          response.body,
        )
      ) {
        const block =
        ...
        if (
          ...
          typeof block.header.hash !==
            "string"
        ) {
          throw new Error(
            "Portal returned a malformed block header",
          );
        }
...
async function main(): Promise<void> {
  ...
  while (running) {
    ...
    try {
      const result =
        await requestStreamBatch(
          ...
          async (block) => {
            ...
            status.instructions +=
              extracted.stats.instructions;

            status.uncommitted +=
              extracted.stats.uncommitted;

            status.decodeFailures +=
              extracted.stats.decodeFailures;

            status.accountLayoutFailures +=
              extracted.stats.accountLayoutFailures;

            status.unknownDiscriminators +=
              extracted.stats.unknownDiscriminators;

            status.missingTransactions +=
              extracted.stats.missingTransactions;

            status.failedTransactions +=
              extracted.stats.failedTransactions;

            status.missingSignatures +=
              extracted.stats.missingSignatures;

            mergeCounts(
              status.discriminatorCounts,
              extracted.discriminators,
            );

            for (
              const event of extracted.events
            ) {
              if (
                !dedupe.addIfNew(
                  event.id,
      ...
      if (
        result.summary.finalizedHeadNumber !==
        undefined
      ) {
        status.finalizedHead =
          result.summary.finalizedHeadNumber;
      }
      ...
      if (
        ...
        204 ||
        result.summary.blocks === 0
      ) {
        status.noDataResponses++;

        if (
          to !== undefined &&
          status.sourceHead >= to
        ) {
      ...
      if (
        ...
        const common =
          recentHeaders.findCommon(
            error.previousBlocks,
          );
      ...
      if (
        ...
        console.warn(
          `[sqd] rate limited; retrying in ${wait}ms`,
        );
      ...
      if (
        ...
        console.error(
          `[sqd] ${errorMessage(error)}`,
        );
      }

      console.warn(
        `[sqd] retrying from slot ${cursor} in ${RETRY_MS}ms`,
      );
