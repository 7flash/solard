...
export const WORKER_SPECS: Record<SolardWorkerName, WorkerSpec> = {
  ...
  "solard-helius-live-v2": {
    ...
    buildId: "helius-live-v8-reserve-price-fallback",
  },
  ...
  "solard-curve-snapshots": {
    ...
    buildId: "curve-snapshots-v8-stale-first-all-feed",
  },
...
export function resolveWorkerNames(): SolardWorkerName[] {
  ...
  const mode = String(process.env.SOLARD_HELIUS_MODE ?? "logs").toLowerCase();
  const list: SolardWorkerName[] = ["solard-server-worker"];

  if (source.includes("pump") || source.includes("both")) {
    list.push("solard-pumpportal-live-v2");
  }
  ...
  if (
    ...
    if (mode === "poll") {
      list.push("solard-helius-live-v2");
    } else if (mode === "laserstream" || mode === "ws") {
      list.push("solard-helius-laserstream-v1");
    } else if (mode === "all") {
      list.push(
        "solard-helius-logs-v1",
        "solard-helius-live-v2",
    ...
    } else if (mode === "logs+poll" || mode === "poll+logs") {
      // Explicit repair mode. Both sources write the same SQLite database, so
      // this should be enabled only when the polling repair stream is needed.
      list.push("solard-helius-logs-v1", "solard-helius-live-v2");
    } else {
      // Default: one Helius trade writer. Running logsSubscribe and HTTP polling
      // together by default duplicates write pressure and can starve indicator
      // and heartbeat writes under Pump bursts.
      list.push("solard-helius-logs-v1");
    }
  }

  // These workers are required for current market cap, holder counts, metadata,
  // and trade-confidence maintenance. Starting only the create/log worker leaves
  // rows visible but permanently unpriced.
  list.push(
    "solard-curve-snapshots",
    "solard-holder-snapshots",
    "solard-metadata-repair",
